...
namespace taqueria.ViewModel
{
    class CarouselIndicators : Grid
        ...
        private Image BuildImage(State state, int position)
        {
            ...
            switch (state)
            {
                case State.Selected:
                    ...
                    //image.WidthRequest = (int)Math.Round(IndicatorWidth * 2);
                    //image.HeightRequest = (int)Math.Round(IndicatorHeight * 2);
                    break;
                case State.Unselect:
                    image.Source = UnselectedImageSource;
                    //image.WidthRequest = IndicatorWidth;
                    //image.HeightRequest = IndicatorHeight;
                    break;
                default:
